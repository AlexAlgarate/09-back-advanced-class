...
class EnvironmentService {
  ...
  load(): void {
    // evita que se vuelvan a cargar variables de entorno si ya se han cargado anteriormente
    // ya que la primera vez, this.environmentVariables es null.
    if (this.environmentVariables) return;

    const currentEnvironment = process.env.NODE_ENV ?? '';
    const environmentFile = this.getEnvironmentFile(currentEnvironment);

    console.log(`${currentEnvironment} - loading - '${environmentFile}'`);
